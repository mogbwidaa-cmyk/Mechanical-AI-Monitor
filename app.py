...
def create_technical_report(factory, machine, vibration, status, fault, temp):
    ...
    RECOMMENDATION:
    {"Immediate Maintenance Required" if vibration > 7.1 else "Continuous Monitoring"}
    
    Generated by Mogahed Smart Systems.
    """
    return report_text
...
with st.sidebar:
    ...
    vibration_val = st.sidebar.slider("الاهتزاز (mm/s)", 0.0, 15.0, 3.2)
...
with c3:
    ...
    st.download_button(label="📥 تحميل التقرير الفني (.txt)", data=report_data, file_name=f"Report_{machine_selected}.txt", mime="text/plain", use_container_width=True)
